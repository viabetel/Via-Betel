...
export function ContaDisponibilidadeClient() {
  const router = useRouter()
  const { user, loading: authLoading } = useAuth()

  useEffect(() => {
    if (!authLoading && !user) {
      router.push("/auth/login?returnTo=/conta/disponibilidade")
    }
      ...
      <div className="container max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-8">
          ...
          <div className="text-center py-12">
            ...
            <p className="text-gray-600">Em breve você poderá configurar seus horários disponíveis para aulas</p>
          </div>
        </div>
      </div>
    </div>
