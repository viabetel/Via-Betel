...
export function ContaPreferenciasClient() {
  const router = useRouter()
  const { user, loading: authLoading } = useAuth()

  useEffect(() => {
    if (!authLoading && !user) {
      router.push("/auth/login?returnTo=/conta/preferencias")
    }
        ...
        <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-8">
          ...
          <div className="text-center py-12">
            ...
            <p className="text-gray-600">
              Em breve você poderá configurar notificações, privacidade e outras preferências
            </p>
          </div>
        </div>
      </div>
